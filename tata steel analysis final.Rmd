...
```{r}
Box.test(LOG_RETURN_NFLX,lag = 20,type = "Ljung")
```
We see that there is correlation between the data so we can apply geometric random motion for analysis
...
for(i in 2: sim.size) 
  tatasteel.sim[i] <- tatasteel_price[i-1]*exp(rt.sim[i])
